...
const lockFreeTreeAlgo = require("../algo/lockfreetree"); //const {createIndexes} = require("./trees.model");
...
async function buyOtherPlayerTree(req, res) {
  try {
    ...
    const playerInfo = await User.findById(playerId); // console.log(treeInfo);
    // console.log(playerInfo);

    if (treeInfo.free == false && treeInfo.locked == false && playerInfo.money >= treeInfo.leave && playerInfo._id != treeInfo.player_id) {
      console.log("tu peux l'acheter");
      const buyATreeNotFree = await otherPlayerPrice(treeInfo, playerInfo);

      if (playerInfo.money >= buyATreeNotFree) {
        //console.log(buyATreeNotFree);
        const updateTree = await Trees.findById(treeId, function (err, doc) {
          doc.free = false;
...
async function lockFreeTree(req, res) {
  try {
    ...
    const treeId = req.body[1];
    const playerId = req.body[0];
    const playerInfo = await User.find({
      _id: playerId
    });
    const treeInfo = await Trees.find({
      _id: treeId
    });
    ...
    if (playerInfo[0].money >= priceForLock) {
      const lockTheTree = await Trees.findById(treeId, function (err, doc) {
        doc.locked = true;
        doc.save();
      ...
      const payThePrice = await User.findById(playerId, function (err, doc) {
...
async function buyAFreeTree(req, res) {
  try {
    ...
    const treeId = req.body[1];
    const playerId = req.body[0];
    const findTree = await Trees.find({
      _id: treeId
    });
    const treeLeave = await findTree[0].leave;
    const user = await User.findById(playerId);

    if (user.money != null && user.money != undefined && user.money >= treeLeave && findTree[0].free == true && findTree[0].locked == false && playerId != findTree[0].player_id) {
      const buyingTree = await Trees.findById(treeId, function (err, doc) {
        doc.player_id = playerId;
        doc.free = false;
      ...
      console.log("Tu n'as pas assez d'argent, vas donc tondre des pelouses !!!!");
    }
  } catch (error) {
    res.send(error);
  }
