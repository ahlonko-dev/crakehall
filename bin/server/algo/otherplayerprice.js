...
async function otherPlayerPrice(treeInfo, playerInfo) {
  ...
  let otherPlayerTreeValiue = 0; // ----- donnée de calcule de l'algo -----
  // valeur de l'arbre ciblé

  const treeLeave = treeInfo.leave; // nombre d'arbres dans un rayon de 100 m : treesInRadius.length
  ...
  let otherPlayerTreeValiueUtile = otherPlayerTreeValiue - treeInfo.leave; // valeur de tout votre arbre dans un rayon de 100m
  ...
  const treePrice = treeLeave + otherPlayerTreeValiueUtile * treesInRadius.length / otherPlayerTree.length + otherPlayerTreeValiueUtile - myTreeInCercleValiue;
  return treePrice;
}

module.exports = otherPlayerPrice; // https://crack-hall-trees.herokuapp.com/trees/buyotherplayertree/5ece7015b467be4c63b04e4a/99
