...
async function lockFreeTreeAlgo(playerInfo, treeInfo) {
  ...
  const radius = 100;
  const treesInRadius = [];
  const playerInRadius = []; // valeur de tous les arbres dans un rayon de 100 m

  let treesInRadiusLeaves = 0; // nombre de joueurs dans un rayon de 100 m] : playerInRadiusUnique.length
  ...
  if (playerInfo._id == treeInfo.player_id) {
    ...
    const playerInRadiusUnique = playerInRadius.reduce(function (acc, valCourante) {
    ...
    const priceForLock = treeLeave * 10 + treesInRadiusLeaves * playerInRadiusUnique.length - playerTreesValiueInRadius / playerInRadiusUnique.length; //console.log(priceForLock);
...
//# sourceMappingURL=lockfreetree.js.map