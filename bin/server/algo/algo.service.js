...
import date from "date-and-time"; // https://www.npmjs.com/package/date-and-time
...
async function updateConnectionDate(id) {
  try {
    ...
    const treesUser = await Trees.find({
      player_id: id
    });
    const leaveToGive = await updateConnectionAlgo(user, treesUser);

    if (leaveToGive != false) {
      const userMoney = Math.floor((user.money + leaveToGive.totalUserLeaveToGive) / leaveToGive.totalLeaveDivision);
